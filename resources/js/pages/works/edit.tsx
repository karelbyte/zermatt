...
import type { ClientOption, Work } from '@/types';
import WorkController from '@/actions/App/Http/Controllers/WorkController';
import { edit, index } from '@/routes/works';
...
export default function WorksEdit({ work, clients }: Props) {
    const breadcrumbs: BreadcrumbItem[] = [
        { title: 'Obras', href: index().url },
        { title: work.name, href: edit(work).url },
    ];

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Editar ${work.name}`} />
            ...
            <div className="space-y-6">
                ...
                <Form
                    action={WorkController.update.url({ work: work.id })}
                    method="put"
                    className="space-y-6"
                >
                    {({ processing, errors }) => (
