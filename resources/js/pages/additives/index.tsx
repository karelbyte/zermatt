...
import { create, edit, index } from '@/routes/additives';

type PaginatedAdditives = {
    data: Additive[];
    current_page: number;
    last_page: number;
    per_page: number;
...
export default function AdditivesIndex({ additives }: Props) {
    ...
    const handleConfirmDelete = () => {
        ...
        setIsDeleting(true);
        router.delete(AdditiveController.destroy(additiveToDelete).url, {
            preserveScroll: true,
            onFinish: () => {
                setIsDeleting(false);
            ...
            <div className="space-y-6">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <Heading
                        variant="small"
                        title="Aditivos"
                        description="Registro de entradas de aditivos por fecha y proveedor"
                    />
                    <Button asChild>
                ...
                {additives.data.length === 0 ? (
                    <EmptyState
                        icon={Package}
                        title="No hay registros de aditivos"
                        description="Aún no has registrado ninguna entrada de aditivos. Crea la primera para comenzar."
                        action={
                            <Button asChild>
                                <Link href={create().url}>
                                    <Plus className="mr-2 size-4" />
                    ...
                    <>
                        ...
                        {additives.last_page > 1 && (
                            <Pagination links={additives.links} className="mt-6" />
                        )}
                    </>
                )}

                <ConfirmModal
                    open={!!additiveToDelete}
                    onOpenChange={(open) => !open && setAdditiveToDelete(null)}
                    title="¿Eliminar registro?"
                    description="Se eliminará este registro de aditivo. Esta acción no se puede deshacer."
