...
import { index } from '@/routes/additives';

type Props = {
    suppliers: { id: number; name: string }[];
};

export default function AdditivesCreate({ suppliers }: Props) {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Nuevo registro de aditivo" />

            <div className="space-y-6">
                <Heading
                    variant="small"
                    title="Nuevo registro de aditivo"
                    description="Registra una entrada de aditivo (fecha, toneladas, documento y opcionalmente proveedor)"
                />

                <Form
                    action={AdditiveController.store.url()}
                    method="post"
                    className="space-y-6"
                    resetOnSuccess
                >
