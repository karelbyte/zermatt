...
import { index } from '@/routes/waterproofings';

type Props = {
    suppliers: { id: number; name: string }[];
};

export default function WaterproofingsCreate({ suppliers }: Props) {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Nuevo registro de impermeabilizante" />

            <div className="space-y-6">
                <Heading
                    variant="small"
                    title="Nuevo registro de impermeabilizante"
                    description="Registra una entrada de impermeabilizante (fecha, cantidad, documento y opcionalmente proveedor)"
                />

                <Form
                    action={WaterproofingController.store.url()}
                    method="post"
                    className="space-y-6"
                    resetOnSuccess
                >
