...
import { index } from '@/routes/fibers';

type Props = {
    suppliers: { id: number; name: string }[];
};

export default function FibersCreate({ suppliers }: Props) {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Nuevo registro de fibra" />

            <div className="space-y-6">
                <Heading
                    variant="small"
                    title="Nuevo registro de fibra"
                    description="Registra una entrada de fibra (fecha, cantidad, documento y opcionalmente proveedor)"
                />

                <Form
                    action={FiberController.store.url()}
                    method="post"
                    className="space-y-6"
                    resetOnSuccess
                >
