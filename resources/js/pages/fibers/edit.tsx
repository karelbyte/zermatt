import { Form, Head, Link } from '@inertiajs/react';
import Heading from '@/components/heading';
import InputError from '@/components/input-error';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';
import AppLayout from '@/layouts/app-layout';
import type { BreadcrumbItem } from '@/types';
import type { Fiber } from '@/types';
import FiberController from '@/actions/App/Http/Controllers/FiberController';
import { edit, index } from '@/routes/fibers';

type Props = {
    fiber: Fiber;
    suppliers: { id: number; name: string }[];
};

function dateInputValue(date: string | null): string {
    if (!date) return '';
    try {
        const d = new Date(date);
        return d.toISOString().slice(0, 10);
    } catch {
        return '';
    }
}

export default function FibersEdit({ fiber, suppliers }: Props) {
    const breadcrumbs: BreadcrumbItem[] = [
        { title: 'Fibras', href: index().url },
        { title: 'Editar registro', href: edit(fiber).url },
    ];

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Editar registro de fibra" />

            <div className="space-y-6">
                <Heading
                    variant="small"
                    title="Editar registro de fibra"
                    description="Modifica fecha, cantidad, documento o proveedor"
                />

                <Form
                    action={FiberController.update.url({ fiber: fiber.id })}
                    method="put"
                    className="space-y-6"
                >
                    {({ processing, errors }) => (
                        <>
                            <div className="grid gap-2">
                                <Label htmlFor="date">Fecha</Label>
                                <Input
                                    id="date"
                                    name="date"
                                    type="date"
                                    defaultValue={dateInputValue(fiber.date)}
                                    className="border-input focus-visible:border-ring focus-visible:ring-ring/50 flex h-9 w-full rounded-md border bg-transparent px-3 py-1 text-base shadow-xs outline-none focus-visible:ring-[3px] md:text-sm"
                                />
                                <InputError message={errors.date} />
                            </div>

                            <div className="grid gap-2">
                                <Label htmlFor="tons">Cantidad (Lt)</Label>
                                <Input
                                    id="tons"
                                    name="lit"
                                    type="number"
                                    step="any"
                                    min="0"
                                    defaultValue={fiber.lit ?? ''}
                                    placeholder="Ejemplo: 100"
                                />
                                <InputError message={errors.lit} />
                            </div>

                            <div className="grid gap-2">
                                <Label htmlFor="supplier_id">Proveedor </Label>
                                <select
                                    id="supplier_id"
                                    name="supplier_id"
                                    className="border-input focus-visible:border-ring focus-visible:ring-ring/50 flex h-9 w-full rounded-md border bg-transparent px-3 py-1 text-base shadow-xs outline-none focus-visible:ring-[3px] md:text-sm"
                                    defaultValue={fiber.supplier_id ?? ''}
                                >
                                    <option value="">Sin proveedor</option>
                                    {suppliers.map((s) => (
                                        <option key={s.id} value={s.id}>
                                            {s.name}
                                        </option>
                                    ))}
                                </select>
                                <InputError message={errors.supplier_id} />
                            </div>

                            <div className="grid gap-2">
                                <Label htmlFor="document">Documento </Label>
                                <Input
                                    id="document"
                                    name="document"
                                    maxLength={255}
                                    defaultValue={fiber.document ?? ''}
                                    placeholder="Documento"
                                />
                                <InputError message={errors.document} />
                            </div>

                            <div className="flex gap-4">
                                <Button type="submit" disabled={processing}>
                                    Guardar cambios
                                </Button>
                                <Button
                                    type="submit"
                                    name="status"
                                    value="closed"
                                    variant="secondary"
                                    disabled={processing}
                                >
                                    Guardar y cerrar
                                </Button>
                                <Button variant="outline" asChild>
                                    <Link href={index().url}>Cancelar</Link>
                                </Button>
                            </div>
                        </>
                    )}
                </Form>
            </div>
        </AppLayout>
    );
}
